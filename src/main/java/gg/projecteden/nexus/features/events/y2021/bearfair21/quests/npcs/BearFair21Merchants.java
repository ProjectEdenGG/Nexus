...
import gg.projecteden.nexus.features.events.y2021.bearfair21.quests.resources.farming.BearFair21FarmingLoot;
...
public class BearFair21Merchants {
	...
	public enum BFMerchant {
			...
			public List<TradeBuilder> getTrades(BearFair21User user) {
				return new ArrayList<>() {{
					if (user.getQuestStage_Lumberjack() == QuestStage.STARTED) {
						add(new TradeBuilder()
							.result(BearFair21MainIsland.getReplacementSaw().clone())
		...
		BOTANIST(BearFair21NPC.BOTANIST) {
			@Override
			public List<TradeBuilder> getTrades(BearFair21User user) {
				return BearFair21FarmingLoot.getTrades();
			}
		},
		COLLECTOR(BearFair21NPC.COLLECTOR) {
			...
			public List<TradeBuilder> getTrades(BearFair21User user) {
				return new ArrayList<>() {{
					if (!Nullables.isNullOrEmpty(BearFair21Collector.getRandomTrades()))
						this.addAll(BearFair21Collector.getRandomTrades());
				}};
			}
		},
			...
			public List<TradeBuilder> getTrades(BearFair21User user) {
				return new ArrayList<>() {{
					...
					if (user.getQuestStage_Main() == QuestStage.STEP_FOUR) {
						add(new TradeBuilder()
							.result(BearFair21MainIsland.getCakeItem().clone())
		...
		JAMES(BearFair21NPC.JAMES) {
			...
			public List<TradeBuilder> getTrades(BearFair21User user) {
				return new ArrayList<>() {{
					add(new TradeBuilder()
						.maxUses(1)
						.result(BearFair21MinigameNightIsland.getCarKey().build())
						.ingredient(goldBlock.clone().amount(1)));
				}};
			}
		},
