...
public enum Pugmas25QuestItem implements QuestItem {
	...
	COMPASS(new ItemBuilder(ItemModelType.EVENT_COMPASS).name("&oCompass").lore("&3Displays Direction")), // to start it, call same method as compass#onjoin
	GPS(new ItemBuilder(ItemModelType.EVENT_GPS).name("&oGPS").lore("&3Displays your direction, area designation, and the current time")),

	FISHING_POCKET_GUIDE(new ItemBuilder(ItemModelType.EVENT_FISHING_POCKET_GUIDE).name("&oFisherman's Pocket Guide").lore("&3Displays your fishing luck")),
	WEATHER_RADIO(new ItemBuilder(ItemModelType.EVENT_WEATHER_RADIO).name("&oWeather Radio").lore("&3Displays the current weather")),
	SEXTANT(new ItemBuilder(ItemModelType.EVENT_SEXTANT).name("&oSextant").lore("&3Displays coordinates")),
	FISH_FINDER(new ItemBuilder(ItemModelType.EVENT_FISH_FINDER).name("&oFish Finder").lore("&3Displays your fishing luck, height, and the current weather", "", "&3Increases fishing luck by &e5")),

	PDA(new ItemBuilder(ItemModelType.EVENT_PDA).name("&oPDA").lore("&3Displays direction, height, area designation, time, weather and fishing luck", "&3Opens the waystone warp menu", "", "&3Increases fishing luck by &e5")),

	FISHING_ROD_WOOD(new ItemBuilder(ItemModelType.FISHING_ROD_WOOD).name("&oWood Fishing Rod").enchant(Enchantment.LURE, 1)),
	FISHING_ROD_REINFORCED(new ItemBuilder(ItemModelType.FISHING_ROD_REINFORCED).name("&oReinforced Fishing Rod").lore("&3Increases fishing luck by &e5").enchant(Enchant.UNBREAKING, 2).enchant(Enchantment.LURE, 2)),
	FISHING_ROD_GOLDEN(new ItemBuilder(ItemModelType.FISHING_ROD_GOLDEN).name("&oGolden Fishing Rod").lore("&3Increases fishing luck by &e15").enchant(Enchantment.LURE, 3).unbreakable().glow()),

	ANGLER_HAT(ArmorSkin.FISHING.apply(new ItemBuilder(Material.LEATHER_HELMET).name("&oAngler Hat").lore("&3Increases fishing luck by &e4").unbreakable())),
	ANGLER_VEST(ArmorSkin.FISHING.apply(new ItemBuilder(Material.LEATHER_CHESTPLATE).name("&oAngler Vest").lore("&3Increases fishing luck by &e4").unbreakable())),
	ANGLER_PANTS(ArmorSkin.FISHING.apply(new ItemBuilder(Material.LEATHER_LEGGINGS).name("&oAngler Pants").lore("&3Increases fishing luck by &e4").unbreakable())),
	...
	GNOMIFIER(new ItemBuilder(ItemModelType.GNOMIFIER).name("&oGnomifier").lore("&3Shrinks the holder to 50% scale").untrashable().unframeable()
