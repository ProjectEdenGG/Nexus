...
public class BearFair21ReflectionGame {
	@Getter
	...
	@Getter
	@Setter
	private static BearFair21ReflectionGameLamp lamp = RandomUtils.randomElement(Arrays.asList(BearFair21ReflectionGameLamp.values()));
	...
	public BearFair21ReflectionGame() {
		new BearFair21ReflectionListener();

		// New Objective
		BearFair21ReflectionGameUtils.newObjective();
	}
	...
	static void startLaser(Player player, BlockFace startFace) {
		active = true;
		BearFair21ReflectionGameUtils.clearLamps();
			...
			if (active) {
				...
				if (middle == 0.5 && !blockType.equals(Material.AIR) && cooldown.get() == 0) {
					...
					if (blockType.equals(Material.REDSTONE_LAMP)) {
						if (BearFair21ReflectionGameUtils.checkObjective(reflections.get(), block.getRelative(0, 1, 0).getType())) {
							...
							BearFair21ReflectionGameUtils.win(reflections.get());
							broadcast = false;
						}
					}
					if (broadcast)
						BearFair21ReflectionGameUtils.broadcastObjective();
					endLaser();
					return;
				}
