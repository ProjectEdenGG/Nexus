...
public class SearchProductsProvider extends ShopProvider {
	...
	public void init(Player player, InventoryContents contents) {
		...
		contents.set(1, 1, ClickableItem.of(nameItem(Material.NAME_TAG, "&6Search by item name"), e -> Nexus.getSignMenuFactory()
		...
		contents.set(1, 3, ClickableItem.of(nameItem(Material.APPLE, "&6Search for food"), e -> {
			browseItemsMenu.getFilters().add(FilterSearchType.SEARCH.of("Food", product -> filter(product.getItem(), MaterialTag.FOODS)));
			browseItemsMenu.open(player);
		}));

		contents.set(1, 5, ClickableItem.of(nameItem(Material.ENCHANTED_BOOK, "&6Search for enchanted items"), e -> {
		...
		contents.set(1, 7, ClickableItem.of(nameItem(Material.DIAMOND_SWORD, "&6Search for tools,", "&6weapons and armor"), e -> {
		...
		contents.set(2, 2, ClickableItem.of(nameItem(Material.POTION, "&6Search for potion materials"), e -> {
		...
		contents.set(2, 4, ClickableItem.of(nameItem(Material.SHROOMLIGHT, "&6Search for light sources"), e -> {
		...
		contents.set(2, 6, ClickableItem.of(nameItem(Material.CYAN_CONCRETE, "&6Search for colored materials"), e -> {
		...
		contents.set(3, 1, ClickableItem.of(nameItem(Material.BLUE_ORCHID, "&6Search for flora"), e -> {
			browseItemsMenu.getFilters().add(FilterSearchType.SEARCH.of("Flora", product -> filter(product.getItem(), MaterialTag.FLORA)));
			browseItemsMenu.open(player);
		}));

		contents.set(3, 3, ClickableItem.of(nameItem(Material.OAK_PLANKS, "&6Search for wooden materials"), e -> {
			browseItemsMenu.getFilters().add(FilterSearchType.SEARCH.of("Wood", product -> filter(product.getItem(), MaterialTag.ALL_WOOD)));
			browseItemsMenu.open(player);
		}));

		contents.set(3, 5, ClickableItem.of(nameItem(Material.DIAMOND_ORE, "&6Search for minerals"), e -> {
		...
		contents.set(3, 7, ClickableItem.of(nameItem(Material.MUSIC_DISC_PIGSTEP, "&6Search for musical items"), e -> {
			browseItemsMenu.getFilters().add(FilterSearchType.SEARCH.of("Musical items", product -> filter(product.getItem(), MaterialTag.MUSIC)));
			browseItemsMenu.open(player);
		}));

		contents.set(4, 2, ClickableItem.of(nameItem(Material.NETHERRACK, "&6Search for nether materials"), e -> {
		...
		contents.set(4, 4, ClickableItem.of(nameItem(Material.END_STONE, "&6Search for end materials"), e -> {
		...
		contents.set(4, 6, ClickableItem.of(nameItem(Material.PRISMARINE_BRICKS, "&6Search for ocean materials"), e -> {
