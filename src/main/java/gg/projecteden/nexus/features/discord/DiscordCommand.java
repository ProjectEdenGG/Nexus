...
public class DiscordCommand extends CustomCommand {
	...
	void id(@Arg("self") OfflinePlayer player) {
		...
		if (Nullables.isNullOrEmpty(user.getUserId()))
	...
	void link(String code) {
		if (Nullables.isNullOrEmpty(code)) {
			if (!Nullables.isNullOrEmpty(user.getUserId())) {
	...
	void linkStatus(@Arg("self") DiscordUser discordUser) {
		...
		if (Nullables.isNullOrEmpty(discordUser.getRoleId()))
	...
	void jda_edit(String channelId, String messageId, String content) {
		Discord.executeOnMessage(channelId, messageId, message -> message.editMessage(content).queue());
	}
	...
	void info() {
		...
		captcha.getUnconfirmed().forEach((id, date) -> {
			String name = Discord.getName(id);
			send("ID: " + name + " / Date: " + TimeUtils.shortDateTimeFormat(date));
		});
	}
	...
	VoiceChannel convertToVoiceChannel(String value) {
		try {
			...
			if (!Nullables.isNullOrEmpty(voiceChannel.getPermission()))
	...
	List<String> tabCompleteVoiceChannel(String value) {
		return Arrays.stream(VoiceChannel.values())
			.filter(voiceChannel -> Nullables.isNullOrEmpty(voiceChannel.getPermission()) || player().hasPermission(voiceChannel.getPermission()))
