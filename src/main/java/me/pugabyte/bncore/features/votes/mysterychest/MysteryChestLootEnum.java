...
public enum MysteryChestLootEnum {
	...
	RewardChestLoot loot;

	MysteryChestLootEnum(String name, ItemStack... items) {
		this.loot = new RewardChestLoot(name, items);
	}

	public RewardChestLoot getLoot() {
		return loot;
	}

	public static RewardChestLoot[] getAllLoot() {
		RewardChestLoot[] list = new RewardChestLoot[values().length];
		for (int i = 0; i < values().length; i++) {
			list[i] = values()[i].getLoot();
		}
		return list;
	}
