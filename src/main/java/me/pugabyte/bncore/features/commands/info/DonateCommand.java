...
public class DonateCommand extends CustomCommand {
	...
	void donate() {
		line();
		json("&3Enjoying the server? &3Share the love by &edonating&3! We are always extremely grateful for donations, and they come with some cool &erewards&3! Visit &ehttps://store.bnn.gg &3to view the packages you can get.||url:https://store.bnn.gg");
		line();
		json(PLUS + "Terms and Conditions||ttp:&3[+] &eClick here before you donate for anything.||cmd:/donate tac");
	}


	@Path("packs")
	void packs() {
		send("&3These are the four basic categories of our donation selection.");
	}
