...
public class PStoneInfoCommand extends CustomCommand {
	...
	void func() {
		...
		line();
		send(json("&3 « &eClick here to return to the Protection Stone menu.").command("/pstoneinfo"));
	}
	...
	void commands() {
		...
		send(json("&3[+] &c/ps allow <player>").hover("&eAllow another player to your edit inside Protection Stone field").suggest("/ps allow "));
		send(json("&3[+] &c/ps remove <player>").hover("&eRemove another player's access to your field").suggest("/ps remove "));
		send(json("&3[+] &c/ps info").hover("&eView information about the field." +
				"\n&3The same output is given by right clicking " +
				"\n&3on a field block with a diamond tool.").suggest("/ps info"));
		send(json("&3[+] &c/ps visualize").hover("&eCreate a temporary glass box around " +
				"\n&eyour field, showing you what is protected." +
				"\n&3Only the blocks &cinside &3the glass are protected.").suggest("/ps visualize"));
		send(json("&3[+] &c/ps take").hover("&eRemove the protection block and place it in your inventory.").suggest("/ps take"));
		send(json("&3[+] &c/ps allowed").hover("&eGet a list of who is allowed to the field").suggest("/ps allowed"));
		send(json("&3[+] &c/ps mark").hover("&eCreate temporary obsidian towers at the " +
				"\n&elocations of your nearby Protection Stones").suggest("/ps mark"));
		send(json("&3[+] &c/ps toggle prevent-flow").hover("&eToggle the water flow prevention flag." +
		...
		send(json("&3[+] &c/freepstone").hover("&eReceive your free monthly Coal PStone").suggest("/freepstone"));
		send(json("&3[+] &c/ps setvelocity <.1-5>").hover("&eSet the power of your Skycannon &3(&e0=auto&3)").suggest("/ps setvelocity "));
		line();
		send(json("&3 « &eClick here to return to the Protection Stone menu.").command("/pstoneinfo"));
	...
	void sap() {
		...
		send("&eProtection Stone Sizes && Prices. &3(&eHover for more info!&3) " +
				"\n");
		send(" &7Coal Ore");
		send(json("      &eSize: &311x11x11").hover("&3(&eRadius of 5&3)"));
		send(json("      &ePrice: &3$650").hover("&3(&e$.50/block&3)"));
		send("" +
				"\n &9Lapis Ore");
		send(json("      &eSize: &321x21x21").hover("&3(&eRadius of 10&3)"));
		send("" +
				"\n &bDiamond Ore");
		send(json("      &eSize: &341x41x41").hover("&3(&eRadius of 20&3)"));
		send(json("      &ePrice: &3$24,100").hover("&3(&e$.35/block&3)"));
		send("" +
				"\n &aEmerald Ore");
		send(json("      &eSize: &381x81x81").hover("&3(&eRadius of 40&3)"));
		send(json("      &ePrice: &3$132,500").hover("&3(&e$.25/block&3)"));
		line();
		send(json("&3 « &eClick here to return to the Protection Stone menu.").command("/pstoneinfo"));
	}
	...
	void help() {
		...
		send(json("&3[+] &eSizes &&e Prices").command("/pstoneinfo sap"));
		send(json("&3[+] &eGetting started").command("/pstoneinfo gs"));
		send(json("&3[+] &ePStone features").command("/pstoneinfo func"));
		send(json("&3[+] &eCommands").command("/pstoneinfo cmds"));
		send(json("&3 « &eClick here to return to the Protection menu.").command("/protection"));
