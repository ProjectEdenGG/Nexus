...
public class ReflectionLaser {
	private final Player player;
	private final Location startLoc;
	private final BlockFace startFace;
	...
	public ReflectionLaser(Player player, Location startLoc, BlockFace startFace, Color laserColor, Material target) {
		this.player = player;
		...
		taskId = Tasks.Countdown.builder()
				...
				.onTick(i -> {
					...
					if (i % Time.SECOND.x(5) == 0)
						curLoc.get().getWorld().playSound(curLoc.get(), Sound.BLOCK_BEACON_AMBIENT, 2F, 1F);
				...
				.onComplete(() -> curLoc.get().getWorld().playSound(curLoc.get(), Sound.BLOCK_BEACON_DEACTIVATE, 2F, 1F))
				.start()
				.getTaskId();
	}

	public void stopLaser(Location location) {
		location.getWorld().playSound(location, Sound.BLOCK_BEACON_DEACTIVATE, 2F, 1F);
		active = false;
		Tasks.cancel(taskId);
	}
